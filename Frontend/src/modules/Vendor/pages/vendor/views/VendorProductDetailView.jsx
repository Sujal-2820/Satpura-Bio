...
export function VendorProductDetailView({ productId, onAddToCart, onBuyNow, onToggleFavourite, favourites = [], onBack, onCartClick }) {
    const { getProductDetails, getRepaymentRules } = useVendorApi()
    const { cart } = useVendorState()
    const dispatch = useVendorDispatch()

    const [product, setProduct] = useState(null)
    const [loading, setLoading] = useState(true)
    const [quantity, setQuantity] = useState(10) // Standard vendor start quantity
    ...
    useEffect(() => {
        const loadInitialData = async () => {
            ...
            try {
                ...
                console.error('Error loading vendor product data:', error)
            } finally {
                setLoading(false)
            }
        }
    ...
    const currentPrice = useMemo(() => {
        if (!product) return 0
        if (selectedAttributeStock) return selectedAttributeStock.vendorPrice || product.priceToVendor || 0
        return product.priceToVendor || 0
    ...
    return (
        <div className="bg-white min-h-screen pb-24">
            ...
            <div className="pt-14 overflow-x-hidden">
                {/* Image Gallery */}
                <div className="relative aspect-square bg-white border-b border-gray-50">
                    <img
                        src={images[selectedImage]}
                        alt={product.name}
                        className="w-full h-full object-contain p-4"
                    />
                    <button className="absolute top-4 right-4 p-2.5 bg-white/80 backdrop-blur-sm rounded-full shadow-lg text-gray-400 border border-gray-100 active:scale-90 transition-transform">
                ...
                <div className="px-1.5 space-y-3">
                    <div className="flex justify-between items-center gap-2 p-3 bg-gradient-to-br from-gray-50 to-white rounded-2xl border border-gray-100 shadow-sm">
                        <div className="space-y-0.5 flex-1 min-w-0">
                            <h2 className="text-base font-semibold text-gray-900 tracking-tight leading-tight line-clamp-2"><TransText>{product.name}</TransText></h2>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {product.attributeStocks?.map((attr, idx) => (
                                ...
                                >
                                    ...
                                    <p className="text-[10px] font-medium mt-0.5 opacity-80">₹{attr.vendorPrice}</p>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    {/* Quantity */}
                    <div className="flex items-center gap-3 bg-gray-50 inline-flex p-1 rounded-full border border-gray-100 shadow-inner">
                        <button
                    ...
                    <div className="space-y-6 pt-2">
                        ...
                        <div className="space-y-3">
                            <h4 className="text-[10px] font-semibold text-gray-500 tracking-widest uppercase ml-1"><Trans>Pricing breakdown</Trans></h4>
                    ...
                    <div className="space-y-3 pt-6 border-t border-gray-100">
                        <h4 className="text-sm font-semibold text-gray-500 tracking-wide"><Trans>Product description</Trans></h4>
                        <div className="text-xs text-gray-600 leading-relaxed product-description-html bg-gray-50/50 p-4 rounded-2xl border border-gray-100">
