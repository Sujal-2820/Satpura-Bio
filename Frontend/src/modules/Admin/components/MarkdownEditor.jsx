...
export function MarkdownEditor({
  ...
  const config = useMemo(() => ({
    ...
    cleanHTML: {
      ...
      denyTags: 'script,style,iframe', // Security
      replaceOldTags: {
        'i': 'em',
        'b': 'strong'
      }
  ...
  const handleChange = (newContent) => {
    // Jodit returns the HTML content directly
    onChange({
      target: {
        name: 'description',
        value: newContent === '<p><br></p>' ? '' : newContent,
      }
    })
  }
  ...
  return (
    <div className={cn(
      ...
      <style>{`
        ...
        /* Remove "Powered by Jodit" if needed, though usually standard */
      ...
      <JoditEditor
        ...
        value={value}
        config={config}
        onBlur={handleChange} // Optimized to save on blur
        onChange={newContent => { }} // We use onBlur for better performance, but track value
      />
