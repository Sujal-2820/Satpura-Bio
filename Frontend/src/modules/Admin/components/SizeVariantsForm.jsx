...
export function SizeVariantsForm({
    ...
    const validate = () => {
        ...
        sizeVariants.forEach(variant => {
            ...
            if (!variant.vendorPrice || parseFloat(variant.vendorPrice) <= 0) {
                newErrors[`${variant.id}_vendorPrice`] = 'Vendor price is required'
            }
            if (!variant.userPrice || parseFloat(variant.userPrice) <= 0) {
                newErrors[`${variant.id}_userPrice`] = 'User price is required'
            }
            if (variant.vendorPrice && variant.userPrice &&
                parseFloat(variant.userPrice) <= parseFloat(variant.vendorPrice)) {
                newErrors[`${variant.id}_userPrice`] = 'User price must be greater than vendor price'
            }
                    ...
                    sizeVariants.map((variant, index) => (
                        ...
                        >
                            ...
                            <div className="grid grid-cols-2 gap-3 mb-3">
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => handleVariantChange(variant.id, 'vendorPrice', e.target.value)}
                                        placeholder="0.00"
                                        min="0"
                                        step="0.01"
                                        className={cn(
                                            'w-full px-3 py-2 rounded-lg border text-sm transition-all focus:outline-none focus:ring-2 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none',
                                            errors[`${variant.id}_vendorPrice`]
                                                ? 'border-red-300 bg-red-50 focus:ring-red-500/50'
                                                : 'border-gray-300 focus:border-green-500 focus:ring-green-500/50'
                                        )}
                                    />
                                    {errors[`${variant.id}_vendorPrice`] && (
                                        <p className="mt-1 text-xs text-red-600">{errors[`${variant.id}_vendorPrice`]}</p>
                                    )}
                                </div>
                                <div>
                                    <label className="block text-xs font-semibold text-gray-700 mb-1">
                            ...
                            <div className="grid grid-cols-2 gap-3 mb-3">
                                <div>
                                    <label className="block text-xs font-semibold text-gray-700 mb-1">
                                        Vendor Discount % <span className="text-gray-400 font-normal">(Optional)</span>
                                    </label>
                                    <div className="relative">
                                        <input
                                            type="number"
                                            value={variant.discountVendor}
                                            onChange={(e) => handleVariantChange(variant.id, 'discountVendor', e.target.value)}
                                            placeholder="0"
                                            min="0"
                                            max="100"
                                            step="0.1"
                                    ...
                                    {variant.discountVendor && variant.vendorPrice && (
                                        <p className="mt-1 text-xs text-green-600">
                                            Final: ₹{(parseFloat(variant.vendorPrice) * (1 - parseFloat(variant.discountVendor) / 100)).toFixed(2)}
                                        </p>
                                    )}
                                </div>
                                <div>
